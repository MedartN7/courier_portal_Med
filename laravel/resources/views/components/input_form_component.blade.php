            ...
            @elseif ( $type === 'direction')
                <div class="row mb-2 {{ $name }}">
                    ...
                    <div class="col-md-3 postfix_container_{{ $name }}">
                        ...
                        <input id="postfix_select_{{ $name }}" type="text" data-message="{{ __( $message ) }}" @if ( $pattern !== '' ) pattern="{{ $pattern }}" @endif  class="form-control @error( "postfix_select_" . $name ) is-invalid @enderror" name="postfix_select_{{ $name }}" value="{{ old( "postfix_select_" . $name ) }}" autocomplete="postfix_select_{{ $name }}" maxlength="6">
                    </div>
                    <div class="col-md-4 direction_city_container_{{ $name }}">
                        ...
                        <input id="direction_city_{{ $name }}" type="text" data-message="{{ __( $message ) }}" @if ( $pattern !== '' ) pattern="{{ $pattern }}" @endif class="form-control @error( "direction_city_" . $name ) is-invalid @enderror" name="direction_city_{{ $name }}" value="{{ old( "direction_city_" . $name ) }}" autocomplete="direction_city_{{ $name }}" maxlength="80">
                    </div>
                </div>
            @else
                {{-- {{ dd($name, $pattern) }} --}}
                <input id="{{ $name }}" type="{{ $type }}" data-message="{{ __( $message ) }}" @if ( $pattern !== '' ) pattern="{{ $pattern }}" @endif class="form_input form-control @error( $name ) is-invalid @enderror" name="{{ $name }}" value="{{ old( $name, $default_data ) }}" {{ $required }} autocomplete="{{ $name }}" maxlength="{{ $maxLength }}" >
            @endif
            @error( $name )
